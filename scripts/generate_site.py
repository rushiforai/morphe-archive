  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .odometer .odigit {
      ...
      line-height: 1.15;
      color: #262b33;
      background: linear-gradient(180deg, #f6f8fa, #e2e7ee);
      border-right: 1px solid #c9cfd8;
    }
    .odometer .odigit:last-child {
      border-right: none;
    }
    .visitor-stats {
      display: flex;
...
<body id="top">
  ...
  <main>
    <div class="wrap">
      ...
      <div class="site-counter" aria-label="Visitor counter">
        ...
        <div class="odometer" id="odometer" aria-label="Visitor count">
          <span class="odigit">0</span><span class="odigit">0</span><span class="odigit">0</span><span class="odigit">0</span><span class="odigit">0</span><span class="odigit">0</span><span class="odigit">0</span>
        </div>
  ...
  <script>
    ...
    detailModalBody.addEventListener("scroll", () => {
      detailModalBody.classList.add("scrolling");
      clearTimeout(bodyScrollTimer);
      bodyScrollTimer = setTimeout(() => detailModalBody.classList.remove("scrolling"), 900);
    }, { passive: true });

    function showDetailModal(headHtml, bodyHtml) {
      detailModalHead.innerHTML = headHtml;
      detailModalBody.innerHTML = bodyHtml;
      detailModalBody.classList.remove("scrolling");
      clearTimeout(bodyScrollTimer);
    ...
    function renderOdometer(value) {
      const digits = [...document.querySelectorAll("#odometer .odigit")];
      const text = String(value == null ? 0 : value).slice(-digits.length);
      digits.forEach((cell, index) => {
        const visible = index >= digits.length - text.length;
        cell.style.display = visible ? "" : "none";
        cell.textContent = visible ? text[index - (digits.length - text.length)] : "";
      });
    }
    ...
    fetch("data.json")
      ...
      .then((data) => {
        ...
        if (visitorStats.total != null) {
          ...
          renderOdometer(visitorStats.total);
        }
          ...
          .then((payload) => {
            ...
            while ((match = cell.exec(overview[1]))) {
              ...
              if (fields[label] && /^\d+$/.test(value)) {
                ...
                if (label === "All") renderOdometer(parseInt(value, 10));
              }
            }
          })
          .catch(() => {});
