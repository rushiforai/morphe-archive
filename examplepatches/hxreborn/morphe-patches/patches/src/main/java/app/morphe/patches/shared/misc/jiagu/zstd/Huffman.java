...
import static app.morphe.patches.shared.misc.jiagu.zstd.BitInputStream.isEndOfStream;
import static app.morphe.patches.shared.misc.jiagu.zstd.BitInputStream.peekBitsFast;
...
import static app.morphe.patches.shared.misc.jiagu.zstd.Util.isPowerOf2;
...
class Huffman
{
    ...
    public int readTable(final byte[] inputBase, final long inputAddress, final int size)
    {
        ...
        int inputSize = getByte(inputBase, input++) & 0xFF;
        ...
        if (inputSize >= 128) {
            ...
            for (int i = 0; i < outputSize; i += 2) {
                int value = getByte(inputBase, input + i / 2) & 0xFF;
    ...
    public void decode4Streams(final byte[] inputBase, final long inputAddress, final long inputLimit, final byte[] outputBase, final long outputAddress, final long outputLimit)
    {
        ...
        long start2 = start1 + (getShort(inputBase, inputAddress) & 0xFFFF);
        long start3 = start2 + (getShort(inputBase, inputAddress + 2) & 0xFFFF);
        long start4 = start3 + (getShort(inputBase, inputAddress + 4) & 0xFFFF);
    ...
    private static int decodeSymbol(byte[] outputBase, long outputAddress, long bitContainer, int bitsConsumed, int tableLog, byte[] numbersOfBits, byte[] symbols)
    {
        ...
        putByte(outputBase, outputAddress, symbols[value]);
