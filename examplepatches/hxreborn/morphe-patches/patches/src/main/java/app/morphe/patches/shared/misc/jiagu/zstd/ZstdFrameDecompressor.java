...
import static app.morphe.patches.shared.misc.jiagu.zstd.BitInputStream.peekBits;
import static app.morphe.patches.shared.misc.jiagu.zstd.ByteArrays.copyMemory;
import static app.morphe.patches.shared.misc.jiagu.zstd.ByteArrays.getByte;
import static app.morphe.patches.shared.misc.jiagu.zstd.ByteArrays.getInt;
import static app.morphe.patches.shared.misc.jiagu.zstd.ByteArrays.getLong;
import static app.morphe.patches.shared.misc.jiagu.zstd.ByteArrays.getShort;
import static app.morphe.patches.shared.misc.jiagu.zstd.ByteArrays.putByte;
import static app.morphe.patches.shared.misc.jiagu.zstd.ByteArrays.putInt;
import static app.morphe.patches.shared.misc.jiagu.zstd.ByteArrays.putLong;
import static app.morphe.patches.shared.misc.jiagu.zstd.Constants.COMPRESSED_BLOCK;
import static app.morphe.patches.shared.misc.jiagu.zstd.Constants.COMPRESSED_LITERALS_BLOCK;
import static app.morphe.patches.shared.misc.jiagu.zstd.Constants.DEFAULT_MAX_OFFSET_CODE_SYMBOL;
import static app.morphe.patches.shared.misc.jiagu.zstd.Constants.LITERALS_LENGTH_BITS;
import static app.morphe.patches.shared.misc.jiagu.zstd.Constants.LITERAL_LENGTH_TABLE_LOG;
import static app.morphe.patches.shared.misc.jiagu.zstd.Constants.LONG_NUMBER_OF_SEQUENCES;
import static app.morphe.patches.shared.misc.jiagu.zstd.Constants.MAGIC_NUMBER;
import static app.morphe.patches.shared.misc.jiagu.zstd.Constants.MATCH_LENGTH_BITS;
import static app.morphe.patches.shared.misc.jiagu.zstd.Constants.MATCH_LENGTH_TABLE_LOG;
import static app.morphe.patches.shared.misc.jiagu.zstd.Constants.MAX_BLOCK_SIZE;
import static app.morphe.patches.shared.misc.jiagu.zstd.Constants.MAX_LITERALS_LENGTH_SYMBOL;
import static app.morphe.patches.shared.misc.jiagu.zstd.Constants.MAX_MATCH_LENGTH_SYMBOL;
import static app.morphe.patches.shared.misc.jiagu.zstd.Constants.MIN_BLOCK_SIZE;
import static app.morphe.patches.shared.misc.jiagu.zstd.Constants.MIN_SEQUENCES_SIZE;
import static app.morphe.patches.shared.misc.jiagu.zstd.Constants.MIN_WINDOW_LOG;
import static app.morphe.patches.shared.misc.jiagu.zstd.Constants.OFFSET_TABLE_LOG;
import static app.morphe.patches.shared.misc.jiagu.zstd.Constants.RAW_BLOCK;
import static app.morphe.patches.shared.misc.jiagu.zstd.Constants.RAW_LITERALS_BLOCK;
import static app.morphe.patches.shared.misc.jiagu.zstd.Constants.RLE_BLOCK;
import static app.morphe.patches.shared.misc.jiagu.zstd.Constants.RLE_LITERALS_BLOCK;
import static app.morphe.patches.shared.misc.jiagu.zstd.Constants.SEQUENCE_ENCODING_BASIC;
import static app.morphe.patches.shared.misc.jiagu.zstd.Constants.SEQUENCE_ENCODING_COMPRESSED;
import static app.morphe.patches.shared.misc.jiagu.zstd.Constants.SEQUENCE_ENCODING_REPEAT;
import static app.morphe.patches.shared.misc.jiagu.zstd.Constants.SEQUENCE_ENCODING_RLE;
import static app.morphe.patches.shared.misc.jiagu.zstd.Constants.SIZE_OF_BLOCK_HEADER;
import static app.morphe.patches.shared.misc.jiagu.zstd.Constants.SIZE_OF_BYTE;
import static app.morphe.patches.shared.misc.jiagu.zstd.Constants.SIZE_OF_INT;
import static app.morphe.patches.shared.misc.jiagu.zstd.Constants.SIZE_OF_LONG;
import static app.morphe.patches.shared.misc.jiagu.zstd.Constants.SIZE_OF_SHORT;
import static app.morphe.patches.shared.misc.jiagu.zstd.Constants.TREELESS_LITERALS_BLOCK;
import static app.morphe.patches.shared.misc.jiagu.zstd.Util.fail;
import static app.morphe.patches.shared.misc.jiagu.zstd.Util.get24BitLittleEndian;
import static app.morphe.patches.shared.misc.jiagu.zstd.Util.mask;
import static app.morphe.patches.shared.misc.jiagu.zstd.Util.verify;
...
class ZstdFrameDecompressor
{
    ...
    static int decodeRawBlock(byte[] inputBase, long inputAddress, int blockSize, byte[] outputBase, long outputAddress, long outputLimit)
    {
    ...
    static int decodeRleBlock(int size, byte[] inputBase, long inputAddress, byte[] outputBase, long outputAddress, long outputLimit)
    {
        verify(outputAddress + size <= outputLimit, inputAddress, "Output buffer too small");

        long output = outputAddress;
        long value = getByte(inputBase, inputAddress) & 0xFFL;
        ...
        for (int i = 0; i < remaining; i++) {
            putByte(outputBase, output, (byte) value);
            output++;
        }
            ...
            long outputAbsoluteBaseAddress)
    {
        ...
        int literalsBlockType = getByte(inputBase, input) & 0b11;
            ...
            long outputAbsoluteBaseAddress)
    {
        ...
        int sequenceCount = getByte(inputBase, input++) & 0xFF;
        if (sequenceCount != 0) {
            if (sequenceCount == 255) {
                verify(input + SIZE_OF_SHORT <= inputLimit, input, "Not enough input bytes");
                sequenceCount = (getShort(inputBase, input) & 0xFFFF) + LONG_NUMBER_OF_SEQUENCES;
                input += SIZE_OF_SHORT;
            }
            else if (sequenceCount > 127) {
                verify(input < inputLimit, input, "Not enough input bytes");
                sequenceCount = ((sequenceCount - 128) << 8) + (getByte(inputBase, input++) & 0xFF);
            }

            verify(input + SIZE_OF_INT <= inputLimit, input, "Not enough input bytes");

            byte type = getByte(inputBase, input++);
    ...
    private static void copyMatchTail(byte[] outputBase, long fastOutputLimit, long output, long matchOutputLimit, long matchAddress, int matchLength, long fastMatchOutputLimit)
    {
        ...
        if (matchOutputLimit < fastMatchOutputLimit) {
            ...
            while (output < matchOutputLimit) {
                putByte(outputBase, output++, getByte(outputBase, matchAddress++));
            }
        }
    }
    ...
    private static long copyMatchHead(byte[] outputBase, long output, int offset, long matchAddress)
    {
        if (offset < 8) {
            ...
            putByte(outputBase, output, getByte(outputBase, matchAddress));
            putByte(outputBase, output + 1, getByte(outputBase, matchAddress + 1));
            putByte(outputBase, output + 2, getByte(outputBase, matchAddress + 2));
            putByte(outputBase, output + 3, getByte(outputBase, matchAddress + 3));
    ...
    private static long copyLiterals(byte[] outputBase, byte[] literalsBase, long output, long literalsInput, long literalOutputLimit)
    {
    ...
    private long computeMatchLengthTable(int matchLengthType, byte[] inputBase, long input, long inputLimit)
    {
        switch (matchLengthType) {
            case SEQUENCE_ENCODING_RLE:
                ...
                byte value = getByte(inputBase, input++);
    ...
    private void executeLastSequence(byte[] outputBase, long output, long literalOutputLimit, long matchOutputLimit, long fastOutputLimit, long literalInput, long matchAddress)
    {
        ...
        while (output < literalOutputLimit) {
            putByte(outputBase, output, getByte(literalsBase, literalInput));
            output++;
            literalInput++;
        }
    ...
    private int decodeCompressedLiterals(byte[] inputBase, final long inputAddress, int blockSize, int literalsBlockType)
    {
        long input = inputAddress;
        ...
        int headerSize;
        int type = (getByte(inputBase, input) >> 2) & 0b11;
        switch (type) {
            case 0:
                singleStream = true;
            case 1: {
                int header = getInt(inputBase, input);
            ...
            case 3: {
                ...
                long header = getByte(inputBase, input) & 0xFF |
                        (getInt(inputBase, input + 1) & 0xFFFF_FFFFL) << 8;
    ...
    private int decodeRleLiterals(byte[] inputBase, final long inputAddress, int blockSize)
    {
        ...
        switch (type) {
            case 0:
            case 2:
                outputSize = (getByte(inputBase, input) & 0xFF) >>> 3;
                input++;
                break;
            case 1:
            ...
            case 3:
                ...
                outputSize = (getInt(inputBase, input) & 0xFF_FFFF) >>> 4;
                input += 3;
                break;
            default:
    ...
    private int decodeRawLiterals(byte[] inputBase, final long inputAddress, long inputLimit)
    {
        ...
        switch (type) {
            case 0:
            case 2:
                literalSize = (getByte(inputBase, input) & 0xFF) >>> 3;
                input++;
                break;
            case 1:
            ...
            case 3:
                ...
                int header = ((getByte(inputBase, input) & 0xFF) |
    ...
    static FrameHeader readFrameHeader(final byte[] inputBase, final long inputAddress, final long inputLimit)
    {
        ...
        int frameHeaderDescriptor = getByte(inputBase, input++) & 0xFF;
        ...
        int windowSize = -1;
        if (!singleSegment) {
            int windowDescriptor = getByte(inputBase, input++) & 0xFF;
        ...
        switch (dictionaryDescriptor) {
            case 1:
                dictionaryId = getByte(inputBase, input) & 0xFF;
                input += SIZE_OF_BYTE;
                break;
            case 2:
                ...
                break;
            case 3:
                dictionaryId = getInt(inputBase, input) & 0xFFFF_FFFFL;
                input += SIZE_OF_INT;
                break;
        }
        ...
        switch (contentSizeDescriptor) {
            case 0:
                if (singleSegment) {
                    contentSize = getByte(inputBase, input) & 0xFF;
                    input += SIZE_OF_BYTE;
                }
                break;
            case 1:
                ...
                break;
            case 2:
                contentSize = getInt(inputBase, input) & 0xFFFF_FFFFL;
                input += SIZE_OF_INT;
                break;
            case 3:
                contentSize = getLong(inputBase, input);
                input += SIZE_OF_LONG;
                break;
        }
    ...
    static int verifyMagic(byte[] inputBase, long inputAddress, long inputLimit)
    {
        ...
        int magic = getInt(inputBase, inputAddress);
