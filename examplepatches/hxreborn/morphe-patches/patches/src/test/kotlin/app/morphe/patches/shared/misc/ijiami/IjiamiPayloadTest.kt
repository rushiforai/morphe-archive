...
internal class IjiamiPayloadTest {
    ...
    @Test
    fun `returns null for a string no method loads`() {
        assertNull(payloadOf(Fixtures.payload(copies = 1)).methodWithStringOrNull(Fixtures.CLASS, "absent string"))
    }

    @Test
    fun `refuses a string that selects more than one method even when null is allowed`() {
        assertFailsWith<PatchException> {
            payloadOf(Fixtures.payload(copies = 3)).methodWithStringOrNull(Fixtures.CLASS, "fixture string 0000")
        }
    }
