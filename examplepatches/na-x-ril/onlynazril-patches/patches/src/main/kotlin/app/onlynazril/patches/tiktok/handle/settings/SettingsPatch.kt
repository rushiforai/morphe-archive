...
private const val SETTINGS_EXTENSION_CLASS_DESCRIPTOR = "Lapp/onlynazril/extension/tiktokHandle/settings/TikTokActivityHook;"
...
private const val SETTINGS_ENTRY_CLASS_DESCRIPTOR =
    "Lapp/onlynazril/extension/tiktokHandle/settings/SettingsEntry;"
...
val settingsPatch = bytecodePatch(
    ...
    execute {
        ...
        fun resolveOpenDebugTargets(): OpenDebugTargets {
            ...
            } ?: throw PatchException("Settings: could not resolve OpenDebug state class from defaultState().")
            ...
            val (composeClassDef, composeMethod) = composeMethods.singleOrNull()
                ?: throw PatchException(
                    "Settings: expected one OpenDebug row compose for $stateClass, " +
        ...
        fun resolveClickWrapperMethod(): MutableMethod {
            ...
            val composeInstructions = composeMutable.implementationOrPatchException("Settings")
                .instructions.toList()
            val wrapperClass = composeInstructions.withIndex().firstNotNullOfOrNull { (index, insn) ->
                ...
                val discriminatorRegister = when (instruction.registerCount) {
                    ...
                    5 -> instruction.registerG
                        ...
                        "Settings: unexpected click wrapper constructor register count ${instruction.registerCount}.",
                    )
                }
            ...
            return matches.singleOrNull() ?: throw PatchException(
                "Settings: expected one OpenDebug click handler in $wrapperClass, " +
                    "found ${matches.size}.",
            )
        }
        ...
        fun resolveOpenDebugFunction2Method(): MutableMethod {
            ...
            val lambdaClass = defaultState.implementation?.instructions?.firstNotNullOfOrNull { insn ->
                ...
                "Settings: could not resolve OpenDebug Function2 lambda class.",
            )
            ...
            return matches.singleOrNull() ?: throw PatchException(
                "Settings: expected one OpenDebug Function2 lambda, found ${matches.size}.",
            )
        }
        ...
        composeMutable.addInstruction(moveResultIndex + 1, "const-string v$titleStringRegister, \"Tweaks\"")
        ...
        OpenDebugCellVmDefaultStateFingerprint.methodOrNull?.let { defaultState ->
            ...
            // id itself comes from the extension, so swapping the glyph never touches bytecode
            // and never depends on a resource id assigned by a particular TikTok build.
            ...
            constructor.addInstructions(
                ...
                """
                    ...
                    invoke-static {}, $SETTINGS_ENTRY_CLASS_DESCRIPTOR->iconResourceId()I
