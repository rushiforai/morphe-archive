...
void main (void)
{
 bool checkAABBmask = true;
	...
	checkAABBmask = false;
#endif
	highp vec4 vTextureDiffuse =vec4(1.0);

#ifdef	GLTEXTURE_FLIP_Y
    vTextureDiffuse  = SampleUITexture(DiffuseTexture, vec2(VSOutTexcoord0_mask.x,1.0 - VSOutTexcoord0_mask.y));
#else
	 vTextureDiffuse  = SampleUITexture(DiffuseTexture, VSOutTexcoord0_mask.xy);
#endif
