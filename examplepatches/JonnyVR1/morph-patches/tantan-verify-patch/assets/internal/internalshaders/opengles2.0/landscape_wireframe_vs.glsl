...
uniform float UsePosition0Directly;
...
{	
	...
	if(UsePosition0Directly > 0.0)
	{
		gl_Position = ViewProjMatrix* WorldMatrix * vec4(Position0.x, Position0.z, Position0.y, 1.0);
	}
	else{
		VertexAttrib vertexAttrib = calcFinalLocalPos(Position0.xy);
		gl_Position = ViewProjMatrix* WorldMatrix * vec4(vertexAttrib.LocalPosition.x, vertexAttrib.LocalPosition.z, vertexAttrib.LocalPosition.y, 1.0);
	}
}