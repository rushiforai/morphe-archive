...
void main (void)
{
...
#else
	mediump vec4 c = texture2D(g_MainTex, UV.xy) * _Color;
#endif
	...
	mediump vec3 CamSpaceLightDir = normalize(viewDirection - _LightDirectionSkin.xyz).xyz;
	...
	highp vec3 specSkin = normalAmount * _SpecColorSkin.rgb * _SpecColorSkin.a * pow(RefDot, _ShininessSkin ) * mask2.r;
			
	c.rgb *= (_LightIntensityAmbientSkin.x * normalAmount + _LightIntensityAmbientSkin.y + _LightIntensityAmbientSkin.z * mask2.g);
