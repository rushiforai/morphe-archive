...
varying vec2		_uv;
varying vec3		_positionWS;
varying vec4		_positionCS;
varying vec3		_normalWS;
varying vec3		_tangentWS;
varying vec3		_binormalWS;
...
void main(void)
{
	...
	_positionCS = gl_Position;
	
	_uv = Texcoord0;	
	_normalWS = normalize((WorldMatrix * normal).xyz);
	_positionWS = (WorldMatrix * pos).xyz;
	_tangentWS = normalize((WorldMatrix * tangent).xyz);
	_binormalWS = normalize(cross(_tangentWS, _normalWS) * Normal0.w);
