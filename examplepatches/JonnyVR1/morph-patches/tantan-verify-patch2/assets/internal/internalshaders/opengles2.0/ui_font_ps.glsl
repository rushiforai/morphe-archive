...
{	
   bool checkAABBmask = true;
	...
	checkAABBmask = false;
#endif
    highp vec2   uv              = compressed.xy;
    highp float  outline         = compressed.z;
    highp float  mask            = compressed.w;

	highp vec4 vTextureDiffuse = SampleUITexture(DiffuseTexture, uv);
...
#endif
   if(compressed.w > 0.1 &&  checkAABBmask)
   {
