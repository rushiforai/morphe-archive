...
// @File         dance_lightprobes_stockings_ps.glsl
...
void main (void)
{
	...
	mediump vec3 LightDirectionLerp = mix(g_LightDirectionSkin.xyz, g_LightDirectionCloth.xyz, cloth_m.g);
	mediump vec3 CamSpaceLightDir = normalize(viewDirection - LightDirectionLerp).xyz;
	highp float LightDot = abs(dot(CamSpaceLightDir, worldN));
	
	mediump float skin_l = (g_LightIntensityAmbientSkin.x * LightDot * 0.6 + g_LightIntensityAmbientSkin.y * 1.2 + g_LightIntensityAmbientSkin.z * skin_m.g);
	mediump float cloth_l = (g_LightIntensityAmbientCloth.x * LightDot * 0.6 + g_LightIntensityAmbientCloth.y * 1.2);
