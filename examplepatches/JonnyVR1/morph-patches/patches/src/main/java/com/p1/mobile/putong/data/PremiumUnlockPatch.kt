...
val premiumUnlockPatch = bytecodePatch(
    ...
    execute {
        ...
        // ── CounterGrowth: complete all growth missions ──
        classDefByOrNull("Lcom/p1/mobile/putong/core/data/CounterGrowth;")?.let { classDef ->
            mutableClassDefBy(classDef).methods.forEach { method ->
                if (method.returnType != "I" || method.parameterTypes.isNotEmpty()) return@forEach
                val instrs = method.cachedInstructions()
                val accessedFields = instrs.filter { instr ->
                    instr is ReferenceInstruction &&
                        instr.reference is FieldReference &&
                        (instr.reference as FieldReference).definingClass == "Lcom/p1/mobile/putong/core/data/CounterGrowth;"
                }.map { ((it as ReferenceInstruction).reference as FieldReference).name }.toSet()
                when {
                    "remaining" in accessedFields -> method.addInstructions(0, RETURN_INT_200000)
                    "total" in accessedFields -> method.addInstructions(0, "const/4 v0, 0x0\nreturn v0")
                }
            }
        }

        // ── FeedFlower: unlimited red flowers for feed ──
        classDefByOrNull("Lcom/p1/mobile/putong/core/data/FeedFlower;")?.let { classDef ->
            mutableClassDefBy(classDef).methods.forEach { method ->
                if (method.returnType != "I" || method.parameterTypes.isNotEmpty()) return@forEach
                val instrs = method.cachedInstructions()
                val accessesCount = instrs.any { instr ->
                    instr is ReferenceInstruction &&
                        instr.reference is FieldReference &&
                        (instr.reference as FieldReference).definingClass == "Lcom/p1/mobile/putong/core/data/FeedFlower;" &&
                        (instr.reference as FieldReference).name == "count"
                }
                if (accessesCount) {
                    method.addInstructions(0, RETURN_INT_200000)
                }
        ...
        // ── LiveUserLevel: maximum live streaming prestige ──
        classDefByOrNull("Lcom/p1/mobile/putong/data/LiveUserLevel;")?.let { classDef ->
            mutableClassDefBy(classDef).methods.forEach { method ->
                if (method.returnType != "I" || method.parameterTypes.isNotEmpty()) return@forEach
                val instrs = method.cachedInstructions()
                val accessedFields = instrs.filter { instr ->
                    instr is ReferenceInstruction &&
                        instr.reference is FieldReference &&
                        (instr.reference as FieldReference).definingClass == "Lcom/p1/mobile/putong/data/LiveUserLevel;"
                }.map { ((it as ReferenceInstruction).reference as FieldReference).name }.toSet()
                when {
                    "grade" in accessedFields -> method.addInstructions(0, RETURN_INT_200000)
                    "gap" in accessedFields -> method.addInstructions(0, "const/4 v0, 0x0\nreturn v0")
                }
            }
        }

        // ── UserWealthGradeConfig: max wealth grade display ──
        classDefByOrNull("Lcom/p1/mobile/putong/data/UserWealthGradeConfig;")?.let { classDef ->
            mutableClassDefBy(classDef).methods.forEach { method ->
                if (method.returnType != "I" || method.parameterTypes.isNotEmpty()) return@forEach
                val instrs = method.cachedInstructions()
                val accessesMinGradeShow = instrs.any { instr ->
                    instr is ReferenceInstruction &&
                        instr.reference is FieldReference &&
                        (instr.reference as FieldReference).definingClass == "Lcom/p1/mobile/putong/data/UserWealthGradeConfig;" &&
                        (instr.reference as FieldReference).name == "minGradeShow"
                }
                if (accessesMinGradeShow) {
                    method.addInstructions(0, "const/4 v0, 0x0\nreturn v0")
                }
            }
        }
