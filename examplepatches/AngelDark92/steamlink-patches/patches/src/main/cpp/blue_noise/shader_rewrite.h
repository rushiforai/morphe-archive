...
inline bool rewrite(std::string_view original, std::string& output,
                    std::string_view configured = configured_shader_hash,
                    std::string_view stock = stock_shader_hash) {
    const auto hash = sha256(original);
    if (hash != configured && hash != stock) return false;
    // These 2 complete source hashes include the exact foveal alpha suffix. Base/opaque,
    // unknown host replacements, already-dithered sources and partial strings never match.
    output = std::string(original);
    const auto main = output.find("void main()");
    if (main == std::string::npos || output.find("void main()", main+1) != std::string::npos) return false;
