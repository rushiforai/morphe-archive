...
Uses the current canonical native resources; UDP adds no native payload. Does not build/sign/install
...
$buildRoot = [IO.Path]::GetFullPath((Join-Path $repo 'build/udp-receive-buffer'))
if (!$OutputDirectory) {
...
$artifactName = "patches-$Version-udp-receive-buffer-local.mpp"
...
fields = {'Manifest-Version':'1.0','Name':'Steam Link GalaxyXR Patches',
 'Description':'Local UDP receive buffer experiment; exact 5002322 and 5002363',
 ...
 'Experimental-Id':'udp-receive-buffer-v1'}
lines=[]
...
assert 'util/UdpReceiveBufferApkAudit.class' in entries
...
'@ | & $Python - $repo $compiled $staged $Version
    ...
    foreach ($apkInput in $inputs) {
        foreach ($selection in @('baseline','baseline-observe','standalone','bundle-first','udp-first','observe-udp-last','udp-first-observe','reapply')) {
            $caseDirectory = Join-Path $run "apks/$($apkInput.Code)-$selection"
            $caseInput = $apkInput.Path
            $arguments = @('-cp',$archiveClasspath,'util.UdpReceiveBufferApkAudit')
            if ($selection -eq 'reapply') { $caseInput = Join-Path $run "apks/$($apkInput.Code)-observe-udp-last/result-unsigned.apk" }
            $arguments += @($staged,$caseInput,$apkInput.Version,$apkInput.Code,$selection,$caseDirectory)
            if ($selection -in @('bundle-first','udp-first')) { $arguments += Join-Path $run "apks/$($apkInput.Code)-baseline/result-unsigned.apk" }
            if ($selection -in @('observe-udp-last','udp-first-observe')) { $arguments += Join-Path $run "apks/$($apkInput.Code)-baseline-observe/result-unsigned.apk" }
    ...
    Copy-Item -LiteralPath $staged -Destination (Join-Path $catalogWork 'build/libs/udp-experiment.mpp')
...
version = sys.argv[3]
patch = 'UDP receive buffer (experimental)'
rows=[]
