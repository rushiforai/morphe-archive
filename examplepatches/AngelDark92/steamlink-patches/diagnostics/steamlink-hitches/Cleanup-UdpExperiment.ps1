...
$allowed = [IO.Path]::GetFullPath((Join-Path $repo 'build/udp-receive-buffer'))
...
$receipt = Join-Path $repo 'diagnostics/steamlink-hitches/udp-receive-buffer-validation.json'
...
if ($validation.artifact.AuditDirectory -ne $root -or $validation.artifact.Sha256 -ne $result.Sha256 -or
