...
then run 16 isolated Morphe APK cases from that archive and generate all catalogs.
Uses the current canonical native resources; FEC guard uses the current diagnostic helper resources. Does not build/sign/install
APKs on a device, use ADB, modify SteamVR, or change Gradle release metadata.
Existing experiment and release MPPs are preserved. Repeated runs use unique workspace output
...
function Assert-ChildPath([string]$Path, [string]$Parent) {
    ...
    while ($ancestor) {
        if ((Test-Path -LiteralPath $ancestor) -and
            ...
            throw "Refusing output/cleanup through reparse ancestor: $ancestor"
        }
...
$artifactName = "patches-$Version-fec-duplicate-reservation-local.mpp"
...
function Remove-CaseTemporary([string]$CaseDirectory) {
    ...
    foreach ($name in @('temporary','isolated-input.apk')) {
        ...
        if (Test-Path -LiteralPath $target) {
            ...
            if ($items | Where-Object { $_.Attributes -band [IO.FileAttributes]::ReparsePoint }) { throw "Refusing cleanup through reparse point: $target" }
...
fields = {'Manifest-Version':'1.0','Name':'Steam Link GalaxyXR Patches',
 'Description':'Local FEC duplicate reservation guard experiment; exact 5002322 and 5002363',
 ...
 'Experimental-Id':'fec-duplicate-reservation-v1'}
lines=[]
...
for c in ('5002322','5002363'):
    for suffix in ('', '_telemetry'):
        assert entries[f'steamlink/decoder/libgxr_dbuf_{c}{suffix}.so'].startswith(b'\x7fELF')
assert 'util/FecDuplicateReservationApkAudit.class' in entries
...
'@ | & $Python - $repo $compiled $staged $Version
    ...
    foreach ($apkInput in $inputs) {
        foreach ($selection in @('baseline','baseline-observe','standalone','bundle-first','guard-first','observe-guard-last','guard-first-observe','reapply')) {
            $caseDirectory = Join-Path $run "apks/$($apkInput.Code)-$selection"
            $caseInput = $apkInput.Path
            $arguments = @('-cp',$archiveClasspath,'util.FecDuplicateReservationApkAudit')
            if ($selection -eq 'reapply') { $caseInput = Join-Path $run "apks/$($apkInput.Code)-observe-guard-last/result-unsigned.apk" }
            $arguments += @($staged,$caseInput,$apkInput.Version,$apkInput.Code,$selection,$caseDirectory)
            if ($selection -in @('bundle-first','guard-first')) { $arguments += Join-Path $run "apks/$($apkInput.Code)-baseline/result-unsigned.apk" }
            if ($selection -in @('observe-guard-last','guard-first-observe')) { $arguments += Join-Path $run "apks/$($apkInput.Code)-baseline-observe/result-unsigned.apk" }
            Invoke-CheckedJava $arguments (Join-Path $run "$($apkInput.Code)-$selection.log")
    ...
    Copy-Item -LiteralPath $staged -Destination (Join-Path $catalogWork 'build/libs/fec-duplicate-experiment.mpp')
...
version = sys.argv[3]
patch = 'FEC duplicate reservation guard (experimental)'
rows=[]
...
'@ | & $Python - $catalogBefore $catalogRoot $Version
    ...
    $summary = [ordered]@{ Status = 'passed'; Artifact = $artifact; Sha256 = (Get-FileHash -LiteralPath $artifact).Hash.ToLowerInvariant();
        AuditDirectory = $run; ApkCases = 16; Inputs = '2 verified original APKs; no reconstructed fallback';
    ...
    # Preserve compact evidence before disposable APK/compiler outputs become cleanup candidates.
    $receipt = Join-Path $repo 'diagnostics/steamlink-hitches/fec-duplicate-reservation-validation.json'
    Assert-ChildPath $receipt $repo
    @'
import sys, json, hashlib
from pathlib import Path
from xml.etree import ElementTree as ET
repo, run, receipt = map(Path, sys.argv[1:4])
artifact=json.loads((run/'result.json').read_text(encoding='utf-8-sig'))
cases=[json.loads(p.read_text(encoding='utf-8-sig')) for p in sorted((run/'apks').glob('*/audit.json'))]
selections={'baseline','baseline-observe','standalone','bundle-first','guard-first','observe-guard-last','guard-first-observe','reapply'}
assert len(cases)==16 and all(c['status']=='passed' and c['stock_udp_receive_buffer_preserved'] is True for c in cases)
assert {(c['version'],c['version_code'],c['selection']) for c in cases}=={(v,c,s) for v,c in [('2.0.22','5002322'),('2.0.23','5002363')] for s in selections}
assert all(c['archive_sha256']==artifact['Sha256'] for c in cases)
tests=dict(tests=0,failures=0,errors=0,skipped=0)
for p in (run/'compiled/test-results').glob('TEST-*.xml'):
    node=ET.parse(p).getroot()
    for field in tests:tests[field]+=int(node.get(field,0))
assert tests['tests']>0 and tests['failures']==tests['errors']==0
paths=[
 'patches/src/main/kotlin/app/template/patches/steamlink/binary/FecDuplicateReservationGuardPatch.kt',
 'patches/src/main/kotlin/util/FecDuplicateReservationApkAudit.kt',
 'patches/src/main/kotlin/app/template/patches/steamlink/binary/DecoderInputBufferingPatch.kt',
 'patches/src/main/kotlin/app/template/patches/steamlink/RecommendedPatchBundles.kt',
 'diagnostics/steamlink-hitches/Build-FecDuplicateExperiment.ps1',
 'diagnostics/steamlink-hitches/Cleanup-FecDuplicateExperiment.ps1',
]
paths += [p.relative_to(repo).as_posix() for p in (repo/'patches/src/main/resources/steamlink/decoder').glob('*.so')]
sources={p:hashlib.sha256((repo/p).read_bytes()).hexdigest() for p in paths}
doc={'date':__import__('datetime').date.today().isoformat(),'experiment':'fec-duplicate-reservation-v1',
     'artifact':artifact,'tests':tests,'sources':sources,'cases':cases,
     'catalog_regression':json.loads((run/'catalog-regression.json').read_text()),
     'runtime':'Not installed or headset-tested by this build'}
receipt.write_text(json.dumps(doc,indent=2)+'\n',encoding='utf-8')
print('PASS canonical validation receipt:',receipt)
'@ | & $Python - $repo $run $receipt
    if ($LASTEXITCODE -ne 0) { throw 'Canonical validation receipt failed; retain audit outputs' }
