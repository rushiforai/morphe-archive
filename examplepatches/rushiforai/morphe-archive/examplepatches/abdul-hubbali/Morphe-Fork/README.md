...
The Google Photos patches in this repository are ported from [De-ReVanced](https://github.com/RookieEnough/De-ReVanced),
which itself is a migration of [ReVanced patches](https://github.com/ReVanced/revanced-patches) to the Morphe ecosystem.
All original patch files include a link to their upstream source as required by GPL v3 Section 5.
...
| ----- | ----------- | ------- |
| `Spoof features` | Spoofs the device as a Google Pixel XL to unlock unlimited original-quality photo backup. | ✅ On |
| `GmsCore support` | Allows the patched app to run without root by using GmsCore instead of Google Play Services. | ✅ On |
| `Extension` | Shared extension required by other Google Photos patches. | ✅ On |
| `Enable DCIM folders backup control` | Disables always-on backup for Camera and DCIM folders, letting you control each folder individually. | ⬜ Off |

> **Note:** `Spoof features`, `GmsCore support`, and `Extension` are required for the patched app to function correctly. `Enable DCIM folders backup control` is optional and off by default.
