package app.chiggi.arrowpuzzle.patches.misc.packagename
...
private const val ORIGINAL_PACKAGE = "com.easybrain.arrow.puzzle.game"

@Suppress("unused")
val changePackageNamePatch = resourcePatch(
    name = "Change package name",
    description = "Changes the app package name so the patched app installs alongside the " +
        "original. Set the desired package name in the patch options. WARNING: this game ships " +
        "native integrity/signing libraries; a renamed + re-signed package may trip a native " +
        "tamper check. Disable this patch if the app crashes or closes on launch.",
    default = true,
) {
    ...
    val packageName by stringOption(
        ...
        title = "Package name",
        description = "The new application package name (e.g. com.easybrain.arrow.puzzle.game.morphe).",
        required = true,
    ) {
    ...
    finalize {
        document("AndroidManifest.xml").use { document ->
            ...
            // androidx-startup, Firebase, ad SDKs, etc.) so they stay unique and do not collide
            // with the original app on install.
