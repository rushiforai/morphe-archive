...
import static app.morphe.patches.shared.misc.jiagu.zstd.FiniteStateEntropy.MAX_SYMBOL;
import static app.morphe.patches.shared.misc.jiagu.zstd.FiniteStateEntropy.MIN_TABLE_LOG;
import static app.morphe.patches.shared.misc.jiagu.zstd.Util.highestBit;
...
class FseTableReader
{
    ...
    public int readFseTable(FiniteStateEntropy.Table table, byte[] inputBase, long inputAddress, long inputLimit, int maxSymbol, int maxTableLog)
    {
        ...
        int position = spreadSymbols(normalizedCounters, maxSymbol, tableSize, highThreshold, table.symbol);
    ...
    private static int spreadSymbols(short[] normalizedCounters, int maxSymbolValue, int tableSize, int highThreshold, byte[] symbols)
    {
