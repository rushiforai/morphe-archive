...
A `scripts/` directory does not currently exist. If scripts are needed that operate outside the Morphe patcher framework (e.g., APK analysis, CI helpers, release tooling), create `scripts/` at the project root and document their usage here.
...
All patch vals must have `@Suppress("unused")` — they are loaded via reflection, not direct references.
