...
public class SignatureHookApp extends Application {
    ...
    private static Parcelable.Creator<PackageInfo> getPackageInfoCreator(String packageName, Signature fakeSignature) {
        ...
        return new Parcelable.Creator<>() {
            ...
            public PackageInfo createFromParcel(Parcel source) {
                ...
                if (packageInfo.packageName.equals(packageName)) {
                    ...
                    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.P && packageInfo.signingInfo != null) {
                        ...
                        if (signaturesArray != null && signaturesArray.length > 0) signaturesArray[0] = fakeSignature;
                        Signature[] historyArray = packageInfo.signingInfo.getSigningCertificateHistory();
                        if (historyArray != null && historyArray.length > 0) {
                            for (int i = 0; i < historyArray.length; i++) historyArray[i] = fakeSignature;
                        }
                    }
                }
                return packageInfo;
            ...
            @Override
            public PackageInfo[] newArray(int size) { return originalCreator.newArray(size); }
        };
    }
