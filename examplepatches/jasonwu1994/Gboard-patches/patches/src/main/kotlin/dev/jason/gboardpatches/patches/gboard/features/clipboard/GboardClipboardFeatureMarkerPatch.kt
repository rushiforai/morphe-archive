...
internal val gboardClipboardFeatureMarkerPatch = resourcePatch(
    description = "標記 clipboard feature 已被打入 target APK，共用 settings UI 過濾"
) {
    compatibleWith(COMPATIBILITY_GBOARD)

    finalize {
        applyClipboardFeatureMarker()
    }
}

context(context: ResourcePatchContext)
private fun applyClipboardFeatureMarker() = with(context) {
    document("AndroidManifest.xml").use { document ->
        ...
        val metaData = application.childElements("meta-data").firstOrNull {
            it.androidAttribute("name") == CLIPBOARD_FEATURE_MARKER_NAME
        } ?: document.createElement("meta-data").also { createdMetaData ->
            application.appendChild(createdMetaData)
        }

        metaData.setAndroidAttribute("name", CLIPBOARD_FEATURE_MARKER_NAME)
