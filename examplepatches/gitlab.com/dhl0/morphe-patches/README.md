...
Fork of [Mix Patches](https://gitlab.com/IMXEren/mix-patches) with an added fix for Sync for
Reddit's native Redgifs playback.

**Why this fork exists:** Mix Patches' "Fix Redgifs API" patch only fixes the app's shared
OkHttp client. Sync's actual click-to-play flow never uses that client — it runs a separate,
hand-written Volley request chain that calls two Redgifs API endpoints that no longer exist. So
previews worked (they're loaded through Sync's own image proxy, bypassing Redgifs entirely) but
tapping a post to play it always failed with "Error connecting to Redgifs", patched or not. This
fork adds **Fix Redgifs native playback**, which redirects that Volley chain's entry point to
reuse the already-working token flow from the OkHttp fix.
...
> **[v1.0.4](https://gitlab.com/dhl0/morphe-patches/-/releases/v1.0.4)**&nbsp;&nbsp;•&nbsp;&nbsp;`main`&nbsp;&nbsp;•&nbsp;&nbsp;13 patches total
<details open>
...
| [Fix Redgifs native playback](#fix-redgifs-native-playback) | Replaces Sync's dead legacy Redgifs OAuth/IP Volley request chain with a working token fetch, so tapping a Redgifs post plays natively instead of showing "Error connecting to Redgifs". |  |
...
Click here to add these patches to Morphe: https://morphe.software/add-source?gitlab=dhl0/morphe-patches

Or manually add this repository url as a patch source in Morphe: https://gitlab.com/dhl0/morphe-patches
