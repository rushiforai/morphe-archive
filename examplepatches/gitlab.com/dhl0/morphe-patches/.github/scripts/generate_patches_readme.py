...
release_url_template = os.environ["PROJECT_RELEASE_URL_TEMPLATE"]
...
def build_content(expanded=False):
    ...
    release_url = release_url_template.format(version=ver)
    lines = [
        f"> **[v{ver}]({release_url})**"
        f"&nbsp;&nbsp;•&nbsp;&nbsp;`{branch}`&nbsp;&nbsp;•&nbsp;&nbsp;"
...
readme = readme.replace("https://morphe.software/add-source?github=xyz-user/xyz-patches", f"https://morphe.software/add-source?gitlab={repo_full}")
readme = readme.replace("https://github.com/xyz-user/xyz-patches", project_url)
