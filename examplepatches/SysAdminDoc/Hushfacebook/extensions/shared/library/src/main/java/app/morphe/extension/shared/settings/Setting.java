...
public abstract class Setting<T> {
     ...
     * Hushfacebook has a context can name the file without loading this class.
     */
    ...
    /**
     * Pause Hushfacebook, decided once when the process starts (see {@link HushfacebookPause}). While it
    ...
    /** Hushfacebook's own state rather than a change to Facebook: it keeps its value while paused. */
    private volatile boolean keptWhenPaused;

    /** Whether this process runs with Hushfacebook paused. */
    ...
    /**
     * Marks settings that hold Hushfacebook's own state (a remembered position, an observed list, a
     * counter, a diagnostics option) so they keep answering their value while Hushfacebook is paused.
    ...
    /**
     * What the setting says now, which is what every hook reads. While Hushfacebook is paused, a
