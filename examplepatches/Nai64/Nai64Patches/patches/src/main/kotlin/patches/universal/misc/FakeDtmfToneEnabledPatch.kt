...
@Suppress("unused")
val fakeDtmfToneEnabledPatch = bytecodePatch(
    name = "Fake DTMF Tone Enabled",
    description = "Reports a chosen DTMF tone state through Settings.System so apps that check dialer tone settings stop doing so.",
    default = false,
) {
    val enabled by booleanOption(
        title = "Enable DTMF tone",
        default = false,
        key = "dtmfTone",
        description = "Report DTMF tone as enabled (true) or disabled (false).",
    )
    ...
    execute {
        ...
        classDefForEach { classDef ->
            ...
            for (method in mutableClass.methods) {
                ...
                for ((index, instruction) in instructions.withIndex()) {
                    ...
                    if (keyValue != "dtmf_tone_enabled") continue
        ...
        if (patched > 0) {
            logger.info("Faked DTMF tone at $patched call site(s)")
        } else {
            logger.warning("No Settings.System dtmf_tone_enabled reads found. No changes applied.")
        }
    }
}
