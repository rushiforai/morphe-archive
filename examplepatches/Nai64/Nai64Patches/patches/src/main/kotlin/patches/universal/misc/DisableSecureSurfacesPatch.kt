...
val disableSecureSurfacesPatch = bytecodePatch(
    ...
    execute {
        ...
        val b = forceBooleanSetter("Landroid/view/SurfaceView;", setOf("setSecure"), false)
        val patched = a + b
        if (patched > 0) {
            logger.info("Forced $patched secure-surface call(s) to false")
        } else {
            logger.warning("No Surface.setSecure() calls found. No changes applied.")
        }
    }
}
