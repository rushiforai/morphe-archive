...
val enableWebViewPopupsPatch = bytecodePatch(
    ...
    execute {
        ...
        val patched = forceBooleanSetter(
            ...
            setOf("setJavaScriptCanOpenWindowsAutomatically", "setSupportMultipleWindows"),
            true,
        )
        if (patched > 0) {
            logger.info("Forced $patched WebSettings popup/window flag(s) to true")
        } else {
            logger.warning("No WebSettings popup/window setters found. No changes applied.")
        }
    }
}
