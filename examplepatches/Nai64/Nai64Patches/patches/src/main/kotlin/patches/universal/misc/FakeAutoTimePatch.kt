...
@Suppress("unused")
val fakeAutoTimePatch = bytecodePatch(
    name = "Fake Auto Time",
    ...
    val enabled by booleanOption(
        ...
        default = true,
        key = "autoTime",
        description = "Report automatic time as enabled (true) or disabled (false).",
    )
    ...
    execute {
        ...
        classDefForEach { classDef ->
            ...
            for (method in mutableClass.methods) {
                ...
                for ((index, instruction) in instructions.withIndex()) {
                    ...
                    if (keyValue != "auto_time") continue
        ...
        if (patched > 0) {
            logger.info("Faked auto time at $patched call site(s)")
        } else {
            logger.warning("No Settings.Global auto_time reads found. No changes applied.")
        }
    }
}
