...
val enableWebViewSaveFormDataPatch = bytecodePatch(
    ...
    execute {
        ...
        val patched = forceBooleanSetter("Landroid/webkit/WebSettings;", setOf("setSaveFormData"), true)
        if (patched > 0) {
            logger.info("Forced $patched setSaveFormData() call(s) to true")
        } else {
            logger.warning("No setSaveFormData() calls found. No changes applied.")
        }
    }
}
