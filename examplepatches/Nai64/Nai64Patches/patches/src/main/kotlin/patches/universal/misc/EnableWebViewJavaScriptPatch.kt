...
val enableWebViewJavaScriptPatch = bytecodePatch(
    ...
    execute {
        ...
        val patched = forceBooleanSetter(
            ...
            setOf("setJavaScriptEnabled"),
            true,
        )
        if (patched > 0) {
            logger.info("Forced $patched setJavaScriptEnabled() call(s) to true")
        } else {
            logger.warning("No setJavaScriptEnabled() calls found. No changes applied.")
        }
    }
}
