...
@Suppress("unused")
val fakeAutoTimeZonePatch = bytecodePatch(
    name = "Fake Auto Time Zone",
    description = "Reports automatic time zone as enabled through Settings.Global so apps that flag manual time zone as suspicious stop doing so.",
    default = false,
) {
    val enabled by booleanOption(
        title = "Enable auto time zone",
        default = true,
        key = "autoTimeZone",
        description = "Report automatic time zone as enabled (true) or disabled (false).",
    )
    ...
    execute {
        ...
        classDefForEach { classDef ->
            ...
            for (method in mutableClass.methods) {
                ...
                for ((index, instruction) in instructions.withIndex()) {
                    ...
                    if (keyValue != "auto_time_zone") continue
        ...
        if (patched > 0) {
            logger.info("Faked auto time zone at $patched call site(s)")
        } else {
            logger.warning("No Settings.Global auto_time_zone reads found. No changes applied.")
        }
    }
}
