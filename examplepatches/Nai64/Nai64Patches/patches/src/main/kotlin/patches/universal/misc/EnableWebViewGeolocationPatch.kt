...
val enableWebViewGeolocationPatch = bytecodePatch(
    ...
    execute {
        ...
        val patched = forceBooleanSetter("Landroid/webkit/WebSettings;", setOf("setGeolocationEnabled"), true)
        if (patched > 0) {
            logger.info("Forced $patched setGeolocationEnabled() call(s) to true")
        } else {
            logger.warning("No setGeolocationEnabled() calls found. No changes applied.")
        }
    }
}
