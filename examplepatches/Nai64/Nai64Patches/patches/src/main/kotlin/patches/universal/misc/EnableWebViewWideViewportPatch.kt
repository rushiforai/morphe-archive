...
val enableWebViewWideViewportPatch = bytecodePatch(
    ...
    execute {
        ...
        val patched = forceBooleanSetter(
            ...
            setOf("setUseWideViewPort", "setLoadWithOverviewMode"),
            true,
        )
        if (patched > 0) {
            logger.info("Forced $patched WebSettings viewport flag(s) to true")
        } else {
            logger.warning("No WebSettings viewport setters found. No changes applied.")
        }
    }
}
