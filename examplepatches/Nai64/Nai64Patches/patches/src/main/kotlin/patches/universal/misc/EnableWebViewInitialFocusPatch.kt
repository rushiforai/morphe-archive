...
val enableWebViewInitialFocusPatch = bytecodePatch(
    ...
    execute {
        ...
        val patched = forceBooleanSetter("Landroid/webkit/WebSettings;", setOf("setNeedInitialFocus"), true)
        if (patched > 0) {
            logger.info("Forced $patched setNeedInitialFocus() call(s) to true")
        } else {
            logger.warning("No setNeedInitialFocus() calls found. No changes applied.")
        }
    }
}
