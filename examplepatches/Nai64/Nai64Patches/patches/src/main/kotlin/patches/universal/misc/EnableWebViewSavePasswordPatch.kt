...
val enableWebViewSavePasswordPatch = bytecodePatch(
    ...
    execute {
        ...
        val patched = forceBooleanSetter("Landroid/webkit/WebSettings;", setOf("setSavePassword"), true)
        if (patched > 0) {
            logger.info("Forced $patched setSavePassword() call(s) to true")
        } else {
            logger.warning("No setSavePassword() calls found. No changes applied.")
        }
    }
}
