...
val allowWebViewAutoplayPatch = bytecodePatch(
    ...
    execute {
        ...
        val patched = forceBooleanSetter(
            ...
            setOf("setMediaPlaybackRequiresUserGesture"),
            false,
        )
        if (patched > 0) {
            logger.info("Forced $patched setMediaPlaybackRequiresUserGesture() call(s) to false")
        } else {
            logger.warning("No setMediaPlaybackRequiresUserGesture() calls found. No changes applied.")
        }
    }
}
