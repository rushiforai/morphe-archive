...
val fakeBrightnessAutoModePatch = bytecodePatch(
    ...
    execute {
        ...
        classDefForEach { classDef ->
            ...
            for (method in mutableClass.methods) {
                ...
                for ((index, instruction) in instructions.withIndex()) {
                    ...
                    if (keyValue != "screen_brightness_mode") continue
        ...
        if (patched > 0) {
            logger.info("Faked brightness auto mode at $patched call site(s)")
        } else {
            logger.warning("No Settings.System screen_brightness_mode reads found. No changes applied.")
        }
    }
}
