...
val enableWebViewImageLoadingPatch = bytecodePatch(
    ...
    execute {
        ...
        val a = forceBooleanSetter("Landroid/webkit/WebSettings;", setOf("setLoadsImagesAutomatically"), true)
        val b = forceBooleanSetter(
            "Landroid/webkit/WebSettings;",
            setOf("setBlockNetworkImage", "setBlockNetworkLoads"),
            false,
        )
        val patched = a + b
        if (patched > 0) {
            logger.info("Forced $patched WebSettings image-loading flag(s)")
        } else {
            logger.warning("No WebSettings image-loading setters found. No changes applied.")
        }
    }
}
