...
val enableWebViewDomStoragePatch = bytecodePatch(
    ...
    execute {
        ...
        val patched = forceBooleanSetter(
            ...
            setOf("setDomStorageEnabled", "setDatabaseEnabled"),
            true,
        )
        if (patched > 0) {
            logger.info("Forced $patched WebSettings storage flag(s) to true")
        } else {
            logger.warning("No WebSettings storage setters found. No changes applied.")
        }
    }
}
