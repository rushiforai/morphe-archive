...
@Suppress("unused")
val customStatusBarColorPatch = resourcePatch(
    name = "Custom Status Bar Color",
    description = "Sets a custom background color for the status bar in every theme",
    default = false,
) {
    category("Interface")
    val barColor by stringOption(
        ...
        default = "#FF000000",
        key = "customStatusBarColor",
        description = "Hex color for the status bar background (e.g. #FF000000 black, #FFFFFFFF white).",
        values = linkedMapOf(
            "Black" to "#FF000000",
    ...
    execute {
        ...
        if (!raw.matches(hexRegex)) {
            logger.warning("Invalid status bar color '$raw'. No changes applied.")
            return@execute
        }
                ...
                runCatching {
                    document(relativePath).use { doc ->
                        ...
                        for (i in 0 until styles.length) {
                            ...
                            ensureThemeItem(doc, style, "android:statusBarColor", raw)
                            changed = true
                        }
        ...
        if (updatedFiles > 0) {
            logger.info("Set status bar color to $raw in $updatedFiles style file(s)")
        } else {
