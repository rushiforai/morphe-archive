...
private const val FLAG_FULLSCREEN = 0x400
...
private data class FlagFullscreenMatch(
...
@Suppress("unused")
val showStatusBarPatch = bytecodePatch(
    name = "Show Status Bar",
    description = "Keep the status bar visible in fullscreen apps",
    default = false,
) {
    category("Interface")
    execute {
        ...
        val matches = mutableListOf<FlagFullscreenMatch>()
        ...
        classDefForEach { classDef ->
            for (method in classDef.methods) {
                ...
                for ((index, insn) in instructions.withIndex()) {
                    ...
                    if ((insn.narrowLiteral and FLAG_FULLSCREEN) == 0) continue
                    ...
                    for (k in index + 1 until instructions.size) {
                        ...
                        val newValue = (insn.narrowLiteral and FLAG_FULLSCREEN.inv()).toUInt()
                        val newSmali = "const v$reg, 0x${newValue.toString(16).uppercase()}"
                        matches += FlagFullscreenMatch(
                            classDef = classDef,
        ...
        for (match in matches) {
            ...
            logger.info("Removed FLAG_FULLSCREEN in ${match.classDef.type} -> ${match.methodName}")
            patched++
        }

        if (patched == 0) {
            logger.warning("No FLAG_FULLSCREEN usages found. No changes applied.")
        } else {
            logger.info("Removed FLAG_FULLSCREEN from $patched method(s)")
        }
    }
}
