...
fun main(args: Array<String>) {
    val expectedVersion = args.singleOrNull()
        ?: error("Expected the patch bundle version as the only argument")
    val patchFile = File("build/libs/patches-$expectedVersion.mpp")
    require(patchFile.isFile) { "Patch bundle not found: ${patchFile.absolutePath}" }

    val manifestVersion = JarFile(patchFile).use { jar ->
        jar.manifest.mainAttributes.getValue("Version")
    }
    require(manifestVersion == expectedVersion) {
        "Patch bundle version $manifestVersion does not match project version $expectedVersion"
    }

    val patchFiles = setOf(patchFile)
    val loadedPatches = loadPatchesFromJar(patchFiles)
    generatePatchList(manifestVersion, loadedPatches)
}
...
private fun generatePatchList(version: String, patches: Set<Patch<*>>) {
    ...
    jsonObject.addProperty(
        "NOTE",
        "Do NOT manually edit this file. This file is automatically generated by " +
                "the generatePatchesList Gradle task. Manually editing this file can break " +
                "third party tools that use it."
    )
