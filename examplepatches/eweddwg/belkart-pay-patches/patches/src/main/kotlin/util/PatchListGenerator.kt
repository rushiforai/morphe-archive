...
private fun generatePatchList(version: String, patches: Set<Patch<*>>) {
    ...
    val patchesMap = patches
        .sortedWith(compareByDescending<Patch<*>> { it.default }.thenBy { it.name })
        .map { patch ->
        JsonPatch(
            name = patch.name!!,
...
/** JSON representation of a compatible app entry, including per-version metadata. */
@Suppress("unused")
private class JsonCompatibility(
    ...
    /** Format as #RRGGBB string; null if not set. */
