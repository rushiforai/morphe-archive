...
private val EMOJI_SIZE_RUNTIME_ABI = RuntimeAbiCatalog.abi(
    RuntimeCallId.EMOJI_SIZE_RUNTIME_APPLY_OVERRIDDEN_FLAG_VALUE,
)

internal val gboardEmojiSizeFlagValuePatch = bytecodePatch(
    ...
    execute {
        ...
        findMutableMethodOrThrow(GboardVersionBindings.flagBoolGetter)
            .applyEmojiSizeFlagValueOverride()
    }
}

internal fun MutableMethod.applyEmojiSizeFlagValueOverride() {
    injectFeatureFlagReturnOverrides(
        runtimeClass = EMOJI_SIZE_RUNTIME_ABI.owner,
        overrideShape = FEATURE_FLAG_OVERRIDE_SHAPE_SHARED,
        unsafeResultRegisters = setOf(0, FEATURE_FLAG_RECEIVER_REGISTER),
        delegateTemplate = EMOJI_SIZE_DELEGATE_TEMPLATE,
    )
}

private val EMOJI_SIZE_DELEGATE_TEMPLATE = """
