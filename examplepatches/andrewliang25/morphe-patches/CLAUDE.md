...
- **A verification tool that ignores an argument will happily verify the wrong file.** `work/Fb.java` takes its dex directory from the **`FBDEX` environment variable**; a `--dex <dir>` argument is accepted silently and ignored, so it falls back to the *unpatched* `work/fb-extract/dex`. That makes a patched site look untouched, which reads exactly like a fingerprint that failed to match. Export `FBDEX` before every post-apply dump, and confirm the dump names the dex you expect. `Fb dump` also does not print `invoke` operand registers, so it cannot show whether an injected call takes the register the `return` reads — check that separately before believing a site is correct.
...
- **`docs/facebook-ads-map.md`** (tracked) covers the Facebook ad surfaces; **`docs/facebook-theme-map.md`** covers Facebook's two colour trees and the AMOLED patch. Facebook strips resource names and renames every class and method about every two weeks, so re-confirm the descriptors on each bump.
