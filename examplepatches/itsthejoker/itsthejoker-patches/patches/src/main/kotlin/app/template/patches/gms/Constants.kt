...
/**
 * Constants for the Roost microG / GmsCore-support port.
 *
 * This is a minimal, self-contained reimplementation of the redirect + signature-spoof
 * technique from Morphe's `shared/misc/gms/GmsCoreSupportPatch`, targeting Roost so a
 * re-signed (Morphe-patched) install can log in with Google via a microG variant
 * ("microG RE") instead of real Google Play Services.
 *
 * The GMS string lists below are copied verbatim from the reference patch; they are
 * generic to Google Play Services, so redirecting a string Roost does not contain is a
 * harmless no-op.
 */
internal object Constants {
    /**
     * microG vendor group id. Morphe/ReVanced-compatible microG (incl. microG RE) registers
     * its services under `<VENDOR>.android.gms` (i.e. `app.revanced.android.gms`).
     */
    const val VENDOR = "app.revanced"

    /** Original Roost package name (what microG must spoof to Google). */
    const val ROOST_PACKAGE_NAME = "com.rookery.courier"

    /**
     * SHA-1 of Roost's original signing certificate, extracted from the untouched
     * base APK's v2 signing block (cert CN=Android, O=Google Inc. — the Play App Signing
     * key the OAuth client is registered against). microG presents this to Google.
     */
    const val SPOOFED_PACKAGE_SIGNATURE = "f4ae0e755c519d34b96e995e8d5d78e1a668682a"

    /** All GmsCore permissions. */
    ...
    /** All GmsCore content provider authorities. */
