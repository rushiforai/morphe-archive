...
# Anchors of the README's own headings. A patch name links to its section only when the README
# has one, so the table never points at a heading that is not there.
README_ANCHORS = set()
...
README_ANCHORS.update(anchor(h) for h in re.findall(r"^#{1,6}\s+(.+?)\s*$", readme, re.M))
