 ...
 *
 * Taken from the official Morphe patches template. Morphe Manager reads
 * patches-list.json from a source repository to show what a bundle contains
 * without downloading it, so this generator has to stay compatible with the
 * upstream schema. Only the NOTE string is changed, because this repository
 * releases on tag rather than through semantic-release.
 */
...
private fun generatePatchList(version: String, patches: Set<Patch<*>>) {
    ...
    jsonObject.addProperty(
        "NOTE",
        "Do NOT manually edit this file. It is regenerated by " +
                "`./gradlew :patches:generatePatchesList` and committed by the release " +
                "workflow. Editing it by hand can break releases and third party tools " +
                "that read it."
    )
