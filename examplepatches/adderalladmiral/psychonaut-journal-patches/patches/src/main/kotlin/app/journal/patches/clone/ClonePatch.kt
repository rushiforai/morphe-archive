package app.journal.patches.clone
...
private const val ORIGINAL_PACKAGE_NAME = "com.isaakhanimann.journal"

@Suppress("unused")
val clonePatch = resourcePatch(
    name = "Clone",
    description = "Renames the package and app label so the patched build installs " +
        "alongside a stock Journal instead of replacing it.",
    default = false,
) {
    ...
    val packageName by stringOption(
        key = "packageName",
        default = "$ORIGINAL_PACKAGE_NAME.morphe",
        title = "Package name",
    ...
    val appName by stringOption(
        key = "appName",
        default = "Journal Morphe",
        title = "App name",
        description = "A new app name (label). Entering \"Journal\" skips changing the label.",
        required = true,
    )
    ...
    execute {
        ...
        document("AndroidManifest.xml").use { document ->
            ...
            // Provider authorities must be globally unique on-device, so the
            // stock Journal and this clone would conflict (and fail to
            // install together) without renaming them here too.
        ...
        if (!appName.isNullOrEmpty() && appName != "Journal") {
            document("res/values/strings.xml").use { document ->
                document.documentElement.forEachChildElement {
                    if (it.textContent == "Journal") {
        ...
        // surfaces (e.g. the RevenueCat integration, file sharing) break in
        // the clone.
