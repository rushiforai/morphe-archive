...
val saveDeletedMessagesPatch =
    bytecodePatch(
        ...
        dependsOn(chatActionBarButtonPatch, directItemEntity, deletedMessagesResourcePatch)
        compatibleWith(COMPATIBILITY_INSTAGRAM)

        execute {
            val nl = Char(10).toString()
            ...
            // Hook 5: remember the thread that is on screen, so a deletion can be attributed to it.
            // The chain viewModel -> descriptor -> DirectThreadKey -> threadId is walked in the
            // extension rather than inline: the builder rarely has a free register below v16, and
            // iget-object / invoke-static encode their registers in 4 bits, so an inline chain is
            // silently dropped. Only the obfuscated names are resolved here.
            runCatching {
                val threadIdField =
                ...
                ChatActionBarBuilderFingerprint.method.apply {
                    ...
                    val afterJoinIndex = joinInstruction.location.index + 1
                    val injectIndex =
                    ...
                    // where it is written when inserting mid-method. move-object/from16 and
                    // invoke-static/range are the only forms that can name a register above v15.
                    addInstructionsWithLabels(
                        injectIndex,
                        listOf(
                            "move-object/from16 v$reg, v$viewModelRegister",
                            "if-eqz v$reg, :piko_no_thread",
                            "invoke-static/range {v$reg .. v$reg}, $HOOK_CLASS->noteOpenThread(Ljava/lang/Object;)V",
