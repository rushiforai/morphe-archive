...
val hideDisplayAdsPatch = bytecodePatch(
    ...
    execute {
        // Return the no-ad singleton (Lsp; = zp.NoAd in Twitch 30.5.0) from the response parser so every
        // display/banner/in-feed unit resolves to "nothing to show". The 204 branch already returns
        // this exact value, so it's a path the downstream code handles cleanly. sp is a sibling of the
        // matched class and version-pinned along with it.
        DisplayAdResponseParserFingerprint.method.addInstructions(
            0,
            """
                sget-object v0, Lsp;->a:Lsp;
                return-object v0
            """,
        )
    }
