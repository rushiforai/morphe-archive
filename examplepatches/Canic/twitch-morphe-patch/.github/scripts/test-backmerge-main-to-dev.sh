...
readonly no_dev_worker="$fixture_root/no-dev-worker"
...
git -C "$seed" push --quiet origin --delete dev
git clone --quiet --branch main "$remote" "$no_dev_worker"
no_dev_output="$({
  cd "$no_dev_worker"
  BACKMERGE_REMOTE=origin bash "$backmerge_script"
} 2>&1)"
grep -q "Remote branch dev does not exist; skipping back-merge." <<< "$no_dev_output"
test "$(git -C "$no_dev_worker" branch --show-current)" = "main"
