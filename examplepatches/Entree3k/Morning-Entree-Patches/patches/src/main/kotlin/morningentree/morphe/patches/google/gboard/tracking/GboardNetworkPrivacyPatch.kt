...
val gboardNetworkPrivacyPatch = bytecodePatch(
    ...
    description = "Controls what Gboard is allowed to talk to. Choose a network level: block only " +
        "telemetry (keep all features), lock it down to GIFs and voice typing only, or block all " +
        "internet access.",
    default = false,
) {
    ...
    val networkLevel by stringOption(
        ...
        title = "Network level",
        description = "How much of Gboard's network access to cut. \"Telemetry only\" keeps every " +
            "feature working. \"Privacy lockdown\" keeps GIF search and cloud voice typing but " +
            "blocks everything else. \"Block all internet\" removes network access entirely.",
        required = true,
