package slight.morphe.patches.at4k.premium
...
import slight.morphe.patches.at4k.license.disableLicenseCheckPatch
...
val unlockPremiumPatch = bytecodePatch(
    ...
    execute {
        ...
        // through the exposed getter by every gate. It is seeded from the "is_premium" pref in
        // launcher_prefs at MainActivity.onCreate and written only by the billing callback through the
        // shared setter (Z)V, which also persists the pref. Force both ends so a free user reads
        // premium on every launch and no billing result can clear it.

        // Pin the purchases manager by unique strings present in the class.
        val purchasesManager = classDefByStrings("Failed to query purchases: ")
            .firstOrNull()
            ?: throw PatchException(
                "AT4K: purchases manager (\"Failed to query purchases: \") not found — the premium " +
                    "flag layout changed.",
            )
        val mutablePurchasesManager = mutableClassDefBy(purchasesManager)

        // The static (Z)V method is the writer: it sets the MutableState and persists is_premium.
        // Force the argument true at entry so the billing callback (which passes false for a non-owner)
        // can never clear the flag, and any direct call lands true.
        val setPremium = mutablePurchasesManager.methods.firstOrNull { method ->
            ...
            ?: throw PatchException(
                "AT4K: static premium setter (Z)V not found — the writer shape changed.",
            )
        ...
        onCreateReturnIndices.forEach { index ->
            onCreate.addInstructions(
                ...
                """
                    const/4 v0, 0x1
                    invoke-static {v0}, ${setPremium.definingClass}->${setPremium.name}(Z)V
                """,
            )
        }
    }
