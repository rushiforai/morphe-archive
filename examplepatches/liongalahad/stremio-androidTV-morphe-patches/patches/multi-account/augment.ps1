...
foreach ($requiredPath in @($javac, $jar, $buildTools, $androidJar, $sourceDirectory)) {
    if (-not (Test-Path -LiteralPath $requiredPath)) {
        throw "Required multi-account module path does not exist: $requiredPath"
    }
}

if ($sources.Count -eq 0) {
    throw "The multi-account module contains no Java sources under $sourceDirectory"
}

$classes = Join-Path $BuildDirectory 'classes'
$dex = Join-Path $BuildDirectory 'dex'
$moduleJar = Join-Path $BuildDirectory 'multi-account.jar'
...
if ($LASTEXITCODE -ne 0) {
    throw 'javac failed to compile the multi-account module.'
}

& $jar cf $moduleJar -C $classes '.'
if ($LASTEXITCODE -ne 0) {
    throw 'Failed to package the multi-account module classes.'
}
...
& (Join-Path $buildTools 'd8.bat') `
    ...
    $moduleJar
if ($LASTEXITCODE -ne 0) {
    throw 'D8 failed to build the multi-account module dex.'
}

Move-Item -LiteralPath (Join-Path $dex 'classes.dex') -Destination (Join-Path $dex 'classes10.dex')

Push-Location $dex
try {
    & $jar uf $artifact 'classes10.dex'
    if ($LASTEXITCODE -ne 0) {
        throw 'Failed to inject the multi-account module dex into the APK.'
    }
}
finally {
    Pop-Location
