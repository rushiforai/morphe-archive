...
if (-not (Test-Path -LiteralPath $patch)) {
    throw "The addon-reordering diff does not exist: $patch"
}
...
try {
    ...
    if ($LASTEXITCODE -ne 0) {
        throw 'The addon-reordering patch is incompatible with this decoded APK.'
    }

    & git apply --ignore-space-change "--directory=$decodedRelative" $patch
    if ($LASTEXITCODE -ne 0) {
        throw 'Failed to apply the addon-reordering patch.'
    }
}
finally {
    Pop-Location
