...
- Morphe uses the same structure as Layout Settings: one native weighted settings-group panel, one native `LazyColumn`, and one native collapsible-section item per category. The native components own each header, description, Open/Closed state, focus treatment, inner padding, row spacing, expansion, and scrolling.
