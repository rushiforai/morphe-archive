...
- [x] Local Downloads applied alone to all four official ABI assets with one exact fingerprint match per hook. The final four combined outputs applied all seven retained patches with zero failures and passed alignment, signature, package, ABI, five-DEX, provider-authority, injected-marker, and forbidden-marker inspection.
- [x] On the API 36 `Television_4K` AVD, the exact final x86_64 APK survived ten cold launches with ten distinct live processes and no fatal exception, verifier error, or ANR. Morphe Settings rendered the Local Downloads section, its enable switch, storage-limit slider, autoplay option, and download list without a crash.
- [x] With all-files access revoked, selecting `Local storage path` opened Android's all-files consent page with Nuvio Morphe shown as not allowed; granting access and returning opened the built-in Internal storage browser.
- [x] A seeded readable Silo S2E1 index entry rendered the `Downloaded` badge on the real episode card in the final release-candidate x86_64 APK at native 4K. Rendered-pixel measurement found both the native watched circle and downloaded circle to be exactly 96×96 px, both 48 px from the card top and their respective outer edge. The corrected hook passes stable `v1` card content and `v11` composer locals, and logs contained no badge-render or fatal error. The 0.8.11 geometry regression check requires the native `na.z0` metrics owner instead of accepting fallback size or inset values.
- [ ] A physical-TV run covering real sources, long downloads, cancellation/resume, storage exhaustion, and greater-than-4-GB FAT32 playback remains a manual release gate.

## Episode menu and ordinary source-picker actions

- [x] Unit tests verify signed-query refreshes retain one source identity, different hosts/paths/addons produce different identities, the fingerprint survives index JSON, and legacy entries remain readable.
- [x] Unit tests verify the ordinary source card starts with exactly `Play` and `Download`, short Play invokes the original source callback once, and download-mode selection records the selected source without changing source resolution.
- [x] Unit tests verify the exact same-source message exposes only `Back`; the exact different-source message exposes `Download and overwrite` and `Back`.
- [x] Unit tests verify overwrite removes the matching indexed episode and arms the replacement source, while another episode in the same season is not a duplicate.
- [x] Isolated patch application verifies the resumed-episode Button/Text, source-card and source-button fingerprints each match exactly once on the official x86_64 APK.
- [x] On the TV AVD, verify Silo S1E1's resumed-playback menu contains `Download to storage`, an ordinary source short press still plays, and a held source opens one Play/Download card without triggering playback on key release.
- [x] On the TV AVD, exercise no-duplicate, same-source and different-source/overwrite episode flows against FAT32 and verify Library > Storage and Settings > Downloads retain one logical row for the exact episode.

## FAT32 segmented-media acceptance

- [x] Unit tests verify one-part raw FAT32 transfers collapse from their temporary bundle into the original video filename, place subtitles beside it, and leave no bundle or manifest.
- [x] Unit tests verify multi-part movies replace the ordinary video with `<filename stem> [short-id]/` inside the normal movie-title folder, while shows place episode bundles directly inside the normal `Show - Season n` folder.
- [x] Unit tests verify move-capable document providers collapse one-part downloads and providers without safe move support retain a valid one-part bundle without data loss.
- [x] Unit tests verify deleting one mirrored bundle removes only that bundle, mixed ordinary and split episodes project into one season, and legacy flat, UUID-only and centralized hidden-store layouts remain readable and deletable.

- [x] Unit tests cover FAT/vfat and exFAT mount classification, exact-boundary rollover without an empty part, manifest validation, logical index reconciliation, whole-bundle deletion, and random reads within and across parts on API 28, 30 and 35.
- [x] Unit tests verify Library > Storage exposes one logical filename/combined size, hides physical parts, attaches the logical file's sidecar and deletes the complete bundle.
- [ ] On a physical FAT32 SD card or USB drive, download a direct progressive file larger than 4 GB and verify every physical part is at most 3,750,000,000 bytes while Settings > Downloads and Library > Storage each show one item.
- [ ] Play the FAT32 item from both surfaces, seek repeatedly immediately before and after every part boundary, select embedded and external subtitle tracks, and verify no timeline transition, subtitle reset, playback error or A/V disruption.
- [ ] Cancel once before and once after the first boundary; verify no manifest, part, `.partial` file, index row or badge remains. Repeat deletion from Settings > Downloads and Library > Storage and verify the whole bundle and sidecars are removed.

## 2026-08-26 FAT32 implementation evidence

- [x] The final suite passed 243 extension tests and four patcher tests with zero failures, errors or skips.
- [x] Source mode is now explicit: a short press in an ordinary Play picker only plays, while a short press in a picker opened through `Download to storage` only downloads. Holding a source exposes `Play` / `Download` in both picker modes.
- [x] Regression coverage starts a held-source download from a Play picker, cancels its active request, short-presses another source in the same picker and verifies the second source bypasses download interception. The inverse case verifies cancellation in a true Download picker keeps its next short-pressed source in Download mode; held Play in that picker bypasses interception once without rewriting the picker mode.
- [x] The corrected combined x86_64 APK reproduced the held-source `Play` / `Download` card in both a normal Silo S01E02 Play picker and an explicit Silo S01E03 Download picker on the visible API 36 TV AVD.
- [x] Silo S1E1's resumed-playback branch now extends the finalized native options list after `CollectionsKt.build`; its real TV AVD menu contains `Download to storage` alongside `Start from beginning`, fixing the branch that previously jumped over the injected action.
- [x] In Silo S1E1's ordinary source picker, a short press retained native playback and a held source opened exactly `Play` / `Download` without an accidental release click. The same-source flow displayed the exact required message with `Back` only; the different-source flow displayed the exact required message with `Download and overwrite` / `Back`.
- [x] A valid different-source overwrite removed the exact owned Silo S1E1 bundle and index row, armed the chosen replacement source and kept the process alive. Unit coverage additionally verifies that an adjacent season episode is preserved.
- [x] The repository-wide isolated matrix applied each of the nine patches to universal, x86_64, arm64-v8a and armeabi-v7a inputs: 36/36 applications passed their manifest, DEX and Smali inspections.
- [x] Combined side-by-side builds applied all nine patches with zero failed patch on all four inputs. Every APK passed ZIP alignment and signature verification, preserved the input's exact native ABI set, contained five non-empty DEX entries, and contained the segmented provider, rolling-download and Local Media runtime classes.
- [x] The final mirrored-layout source also applied Local Downloads alone and Local Media alone to x86_64, universal, arm64-v8a and armeabi-v7a with exactly one applied patch and no failed patch in each of the eight isolated runs.
- [x] The combined x86_64 build installed and launched on the API 36 `Television_4K` AVD. `com.nuvio.morphe/com.nuvio.tv.MainActivity` was resumed, the private `com.nuvio.morphe.morphe.segmented` provider was registered, and startup logs contained no fatal exception, verifier error, missing class, provider-instantiation failure or app ANR.
- [x] Boundary-read tests verify all physical segment handles are opened before playback starts, the combined descriptor returns exact bytes within and across boundaries, and random/tail reads preserve one continuous logical offset space on API 28, 30 and 35.
- [x] Android SDK `mksdcard` created a 6 GiB FAT32 image mounted by the API 36 `Television_4K` AVD as `/dev/block/vold/public:253,80` with filesystem type `vfat`; an attempted 4,294,967,296-byte file failed with `File too large`.
- [x] A byte-exact 64,657,027-byte H.264/AAC fixture was split into 16 parts on that FAT32 volume and exposed as one 65 MB Library > Storage item. Device-side concatenation reproduced the source SHA-256 `f78f39603e6774907f2faafabf26a667f4a6fc31769ec304a8a8f7c62d280508`.
- [x] The FAT32 completion seeder emulated a finished Silo S01E01 download in the production show layout, `Silo - Season 1/S01E01 - Freedom Day [fat32sil]/`, with 16 physical parts, a sidecar subtitle, manifest and exact episode index identity. Library > Storage projected one `Silo - S01E01 - Freedom Day.mp4` item and Settings > Downloads projected one `Silo · S01E01` row; neither surface exposed the parts.
- [x] The final Silo episode playback ran for 168.131 seconds with its downloaded subtitle sidecar attached and crossed seven observed part reads. The structured summary reported `stalls=0`, `stutters=0`, `maxStallMs=0`, `droppedFrames=0`, `slowReads=0`, `errors=0`, `maxBoundaryReadUs=1512`, and `result=NO_PLAYER_STUTTER_DETECTED`.
- [x] Instrumented ExoPlayer playback ran continuously for 159.88 seconds across five physical part boundaries. The final trace reported `stalls=0`, `stutters=0`, `droppedFrames=0`, `errors=0`, `maxBoundaryReadUs=1794`, and `result=NO_PLAYER_STUTTER_DETECTED`.
- [x] An earlier centralized-layout candidate crossed four more boundaries during 96.6 seconds of continuous playback. Its delayed teardown summary reported `stalls=0`, `stutters=0`, `droppedFrames=0`, `errors=0`, `maxBoundaryReadUs=2119`, and `result=NO_PLAYER_STUTTER_DETECTED`; the historical trace is retained under `local/verification/fat32-readable-layout-final2-all-nine-20260826`.
- [x] The final mirrored-layout APK displayed a normal `Single Part Sample/BigBuckBunny_320x180.mp4` and split `Big Buck Bunny/BigBuckBunny_320x180 [fat32bbb]/` as visually identical one-file Library folders. Both reconstructed the same source SHA-256, and the ordinary file reached Media3 `PLAYING` with `error=null`.
- [x] Final mirrored multi-part playback ran for 77.24 seconds across three boundaries and reported `stalls=0`, `stutters=0`, `droppedFrames=0`, `slowReads=0`, `errors=0`, `maxBoundaryReadUs=34`, and `result=NO_PLAYER_STUTTER_DETECTED`. The trace is retained under `local/verification/fat32-mirrored-layout-all-nine-20260826`.
- [x] The same-basename external SRT appeared as one `BigBuckBunny_320x180.en.srt` track and rendered a cue during segmented playback. The trace, report and screenshot are retained in `local/verification/fat32-playback-20260826-111038`.
- [x] Duplicate logical filenames reserve both ordinary files and prior segmented manifests, preventing a later download from replacing an earlier bundle manifest.
- [x] The final isolated Local Downloads patch and combined all-nine patch each applied successfully on x86_64, arm64-v8a, armeabi-v7a and universal inputs. The source-mode correction's combined outputs passed alignment, signature, DEX and exact-native-ABI inspection; SHA-256 values are x86_64 `92A69BAB61045CC9F0C10D4E9A154D168DC89DFA4CAD247DEC4FDBC2A63D9B7B`, arm64-v8a `1B6B98EF37AD35FEA428B5F1EAFAF0D85296FD5B7DC2A4848FBCBE7911410D74`, armeabi-v7a `7CD3C7FDA67B20B4EEA691BE07DBAE87F7C6B375DB2563D31AE0E64DB3F3B964`, and universal `A6ECC37FA76AFC685B6C9D25372135FE794797550D02B6F500847451A55EE727`.
- [x] The final ARM32 user-test APK overwrote `G:\My Drive\Nuvio-Morphe-0.8.7-beta-FAT32-segmented-test-armeabi-v7a-20260826.apk`; source and destination SHA-256 are `7CD3C7FDA67B20B4EEA691BE07DBAE87F7C6B375DB2563D31AE0E64DB3F3B964`.
- [ ] A physical FAT32 greater-than-4-GB download, repeated seeking around the production 3,750,000,000-byte boundary, hardware-specific A/V validation and cleanup cases above remain the release gate. The AVD test used a real FAT32 filesystem but deliberately smaller test boundaries to exercise several transitions quickly.
...
- The episode-card hook resolves and preserves Nuvio's live card-content and Compose-composer registers before either parameter register is reused, then calls `renderDownloadedEpisodeBadge(Object, Object)` with those stable locals. Post-patch Smali inspection rejects the obsolete `p0..p2` call shape.
- For that historical 0.8.7 candidate, local download source resolution used the then-current `c0.u.a` and `w.n.g` contracts. The 0.8.10 port uses the verified `c0.u.a` and `w.m.g` contracts, and its post-patch Smali gates require both target contracts and the corrected episode-card call before an APK is accepted.
