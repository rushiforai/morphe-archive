...
- [x] Run all extension unit tests, including settings defaults, percentage arithmetic, route identity, source eligibility, player-route persistence and Local Media tagged sidecars.
...
- [x] Verify `Maximum usable internal storage` uses the native slider, maps 1, 5, 10 ... 95, 100, defaults to 80, and updates at the native control speed.
- [x] Verify the settings row and progress popup both show one-decimal used/total internal GB and parenthesized percent full.
...
- On the TV AVD, `Maximum usable internal storage` rendered as the native slider at 80%, moved 80 -> 75 -> 80 with one D-pad step, and showed `4.9 GB used / 5.8 GB total (85% full)`. The same storage caption was verified in the download progress popup.
