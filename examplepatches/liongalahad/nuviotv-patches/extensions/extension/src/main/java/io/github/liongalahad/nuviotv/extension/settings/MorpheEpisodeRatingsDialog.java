...
/** TV choice dialog matching Nuvio's single-choice settings layout. */
final class MorpheEpisodeRatingsDialog {
    ...
    private MorpheEpisodeRatingsDialog() {}

    static void show(Activity activity) { create(activity).show(); }
    ...
    static Dialog create(Activity activity) {
        ...
        content.setLayoutParams(new ViewGroup.LayoutParams(dp(activity, DIALOG_WIDTH_DP),
                ViewGroup.LayoutParams.WRAP_CONTENT));

        TextView heading = text(activity, MorpheSettingsRuntime.episodeRatingsTitle(), 24, TEXT_PRIMARY);
        heading.setTypeface(Typeface.DEFAULT, Typeface.NORMAL);
        content.addView(heading, new LinearLayout.LayoutParams(
                ViewGroup.LayoutParams.MATCH_PARENT, ViewGroup.LayoutParams.WRAP_CONTENT));

        int selected = MorpheSettingsRuntime.episodeRatingsModeOrdinal();
        ModeOptionView first = null;
        for (int mode = MorpheSettingsRuntime.EPISODE_RATINGS_SHOW;
             mode <= MorpheSettingsRuntime.EPISODE_RATINGS_HIDE_UNWATCHED; mode++) {
            ModeOptionView option = new ModeOptionView(activity, mode, mode == selected, dialog);
            LinearLayout.LayoutParams params = new LinearLayout.LayoutParams(
                    ViewGroup.LayoutParams.MATCH_PARENT, ViewGroup.LayoutParams.WRAP_CONTENT);
            params.topMargin = dp(activity,
                    mode == MorpheSettingsRuntime.EPISODE_RATINGS_SHOW ? 16 : 8);
    ...
    private static final class ModeOptionView extends LinearLayout {
        ...
        ModeOptionView(Activity activity, int mode, boolean selected, Dialog dialog) {
            ...
            title = text(activity, MorpheSettingsRuntime.episodeRatingsModeTitle(mode), 18,
                    selected ? SELECTED_CONTENT : TEXT_PRIMARY);
            description = text(activity, MorpheSettingsRuntime.episodeRatingsModeDescription(mode), 14,
            ...
            setOnClickListener(view -> {
                MorpheSettingsRuntime.setEpisodeRatingsMode(mode);
        ...
        @Override protected void onFocusChanged(boolean focused, int direction, android.graphics.Rect oldRect) {
            super.onFocusChanged(focused, direction, oldRect);
        ...
        private void updateBackground(boolean focused) {
            ...
            description.setTextColor(focused ? TEXT_SECONDARY :
                    (selected ? SELECTED_CONTENT : TEXT_SECONDARY));
        }
    }
